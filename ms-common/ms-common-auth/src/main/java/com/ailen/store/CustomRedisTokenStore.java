...
import com.anshare.base.common.oauth2.properties.SecurityProperties;
...
/**
 * 优化Spring Security的RedisTokenStore
 * 自动续签token（可配置是否开启）
 *
 * @author anshare
 * 
...
public class CustomRedisTokenStore implements TokenStore {
            ...
            if (checkRenewClientId(clientAuth.getClientId())) {
                ...
                if (validitySeconds > 0) {
                    ...
                    if (expiresRatio <= securityProperties.getAuth().getRenew().getTimeRatio()) {
                        ...
                        log.info("redis: accessToken 自动续期");
                    }
                }
            }
        }
    ...
    private int getAccessTokenValiditySeconds(String clientId) {
        ...
        byte[] bytes;
        try {
            bytes = conn.get(serializeKey(CACHE_CLIENT_KEY + ":" + clientId));
        } finally {
            conn.close();
        }
        if (bytes != null) {
    ...
    public void removeAccessToken(String tokenValue) {
        ...
        try {
            ...
            if (authentication != null) {
                ...
                byte[] clientId = serializeKey(CACHE_CLIENT_KEY + authentication.getOAuth2Request().getClientId());
                conn.openPipeline();
    ...
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        byte[] key = serializeKey(CACHE_CLIENT_KEY + clientId);
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try {
