...
public class MyRedisTokenStore implements TokenStore {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        // client_credentials 模式: 重复获取token后会使上次的token失效(但有5分钟的缓冲时间)
        if (authentication.getOAuth2Request().getGrantType().equals("client_credentials")) {
            if (accessToken != null) {
                DefaultOAuth2AccessToken defaultOAuth2AccessToken = (DefaultOAuth2AccessToken) accessToken;
                log.info("原有: value:{},expiresIn:{},expiration:{}", defaultOAuth2AccessToken.getValue(), defaultOAuth2AccessToken.getExpiresIn(), defaultOAuth2AccessToken.getExpiration());
                defaultOAuth2AccessToken.setExpiration(new Date(System.currentTimeMillis() + (5 * 60 * 1000)));
                log.info("更新: value:{},expiresIn:{},expiration:{}", defaultOAuth2AccessToken.getValue(), defaultOAuth2AccessToken.getExpiresIn(), defaultOAuth2AccessToken.getExpiration());
