...
import com.ailen.annotation.Lock;
...
public class LockAop {
    ...
    /**
     * 环绕方式处理，
     * @param proceedingJoinPoint proceed()这个方法，就能支持 aop:around 这种切面
     * @param lock
     * @return
     * @throws Throwable
     */
...
//        @Lock(lockModel = LockModel.WRITE, keys = "right-auto-refresh")
//        使用类似这种，确定锁模式，和key名字
        String[] keys = lock.keys();
        if (keys.length == 0) {
            throw new RuntimeException("keys不能为空");
        }
        //使用Spring的LocalVariableTableParameterNameDiscoverer获取被@lock标注的方法参数名称
        ...
        switch (lockModel) {
            ...
            case REDLOCK:
                ...
                for (String key : keys) {
                    List<String> valueBySpel = getVauleBySpel(key, parameterNames, args, lock.keyConstant());
            ...
            case MULTIPLE:
                ...
                break;
            case REENTRANT:
                List<String> valueBySpel = getVauleBySpel(keys[0], parameterNames, args, lock.keyConstant());
