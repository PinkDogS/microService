...
public class RestTemplateUtil {
     ...
     * @return
     */
    public static InputStream downloadFile(String url, String paramData, String sessionId) throws IOException {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        if (StrUtil.isNotEmpty(sessionId)) {
            List<String> cookies = new ArrayList<>();
        ...
        HttpEntity<String> requestEntity = new HttpEntity<>(paramData, headers);
        ResponseEntity<Resource> entity = restTemplate.postForEntity(url, requestEntity, Resource.class);
        return entity.getBody().getInputStream();
    }
